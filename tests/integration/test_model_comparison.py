"""Integration tests for single_track versus point-mass model comparison."""
...
class ModelComparisonTests(unittest.TestCase):
    ...
    def test_single_track_and_point_mass_models_both_run_in_same_solver(self) -> None:
        ...
        self.assertTrue(np.isfinite(single_track_result.lap_time))
        self.assertTrue(np.isfinite(point_mass_result.lap_time))
        self.assertGreater(single_track_result.lap_time, 60.0)
    ...
    def test_calibrated_point_mass_matches_single_track_more_closely(self) -> None:
        ...
        lap_delta_default = abs(single_track_result.lap_time - default_result.lap_time)
        lap_delta_calibrated = abs(single_track_result.lap_time - calibrated_result.lap_time)
        ...
        default_speed_delta = np.abs(
            single_track_result.speed[segment_mask] - default_result.speed[segment_mask]
        )
        calibrated_speed_delta = np.abs(
            single_track_result.speed[segment_mask] - calibrated_result.speed[segment_mask]
        )
