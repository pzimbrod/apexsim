...
if TYPE_CHECKING:
    from apexsim.vehicle.point_mass_model import (
...
def __getattr__(name: str) -> Any:
    ...
    if name == "SingleTrackPhysics":
        from apexsim.vehicle.single_track_model import SingleTrackPhysics

        return SingleTrackPhysics
    if name == "SingleTrackNumerics":
        from apexsim.vehicle.single_track_model import SingleTrackNumerics

        return SingleTrackNumerics
    if name == "build_single_track_model":
        from apexsim.vehicle.single_track_model import build_single_track_model

        return build_single_track_model
    if name == "PointMassModel":
        from apexsim.vehicle.point_mass_model import PointMassModel

        return PointMassModel
    if name == "PointMassPhysics":
        from apexsim.vehicle.point_mass_model import PointMassPhysics

        return PointMassPhysics
    if name == "build_point_mass_model":
        from apexsim.vehicle.point_mass_model import build_point_mass_model

        return build_point_mass_model
    if name == "PointMassCalibrationResult":
        from apexsim.vehicle.point_mass_model import PointMassCalibrationResult

        return PointMassCalibrationResult
    if name == "calibrate_point_mass_friction_to_single_track":
        from apexsim.vehicle.point_mass_model import calibrate_point_mass_friction_to_single_track
