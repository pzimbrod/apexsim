...
def _is_single_track_model(model: Any) -> bool:
    ...
    """
    return (
        hasattr(model, "_magic_formula_lateral_torch")
        and hasattr(model, "_axle_tire_loads_torch")
...
def _build_control_node_count(*, sample_count: int, control_interval: int) -> int:
    """Return number of control nodes used in transient optimization.
...
def _single_track_derivatives_torch(
    *,
    ...
    cg_to_front = float(model.vehicle.cg_to_front_axle)
    cg_to_rear = float(model.vehicle.cg_to_rear_axle)

    alpha_front = steer - torch.atan2(vy + cg_to_front * yaw_rate, u)
    alpha_rear = -torch.atan2(vy - cg_to_rear * yaw_rate, u)

    speed = torch.clamp(torch.abs(vx), min=0.0)
    front_tire_load, rear_tire_load = model._axle_tire_loads_torch(speed)
    fy_front = 2.0 * model._magic_formula_lateral_torch(
        torch=torch,
        slip_angle=alpha_front,
        normal_load=front_tire_load,
        params=model.tires.front,
    )
    fy_rear = 2.0 * model._magic_formula_lateral_torch(
        torch=torch,
...
def solve_transient_lap_torch(
    *,
    ...
    if config.transient.runtime.driver_model == "pid":
        ...
        numpy_profile = solve_transient_lap_numpy(track=track, model=model, config=config)
        return _from_numpy_profile(
            torch=torch,
            profile=numpy_profile,
            device=config.runtime.torch_device,
        )
