...
from pylapsim.track.models import MIN_TRACK_POINT_COUNT, TrackData
...
def _validate_positive(name: str, value: float) -> None:
    ...
    Raises:
        pylapsim.utils.exceptions.TrackDataError: If ``value`` is not
            strictly positive.
    """
    if value <= 0.0:
...
def _validate_sample_count(sample_count: int) -> None:
    ...
    Raises:
        pylapsim.utils.exceptions.TrackDataError: If ``sample_count`` is
...
def build_straight_track(
    ...
    Raises:
        pylapsim.utils.exceptions.TrackDataError: If geometric input values are
