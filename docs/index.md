# PyLapSim Documentation

PyLapSim is a modular lap-time simulation library for race cars with
interchangeable vehicle models.

## Documentation Structure

- **Guides**: onboarding, solver derivation, assumptions, development notes.
- **Examples**: step-by-step script walkthroughs (synthetic tracks first, then Spa).
- **API Reference**: auto-generated reference from Python docstrings.

## Quick Links

- [How to Use](HOW_TO_USE.md)
- [Compute Backends](BACKENDS.md)
- [API Overview](API.md)
- [Examples Overview](EXAMPLES.md)
- [Synthetic Track Walkthrough](EXAMPLES_SYNTHETIC.md)
- [Spa Walkthrough](EXAMPLES_SPA.md)
- [Solver Derivation](SOLVER.md)
- [Single-Track Model](SINGLE_TRACK_MODEL.md)
- [Point-Mass Model](POINT_MASS_MODEL.md)
- [Assumptions](ASSUMPTIONS.md)
